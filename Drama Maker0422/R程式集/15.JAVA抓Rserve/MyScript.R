...
dt8=data.frame(t(dt7),row.names = NULL)
colnames(dt8) <- colmame89

x1=read.csv("C:/Users/ShihPing/Desktop/catetable.csv")
x1=na.exclude(x1)
x1.tree=naiveBayes(x1$imdb ~ . , data=x1)
pred <- predict(x1.tree, x1)
score<- predict(x1.tree,dt8)
